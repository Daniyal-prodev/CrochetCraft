// Get all patterns (sample + admin added)
function getAllPatterns() {
    ...
    // Combine both arrays
    return [...samplePatterns, ...adminProducts];
}
...
let currentPatterns = getAllPatterns();
...
document.addEventListener('DOMContentLoaded', () => {
    currentPatterns = getAllPatterns();
    displayPatterns();
    updateCartCount();
    
    // Also update home page patterns if on home page
    if (window.location.pathname === '/' || window.location.pathname.includes('index.html')) {
        updateHomePatterns();
    }
});

// Update home page patterns
function updateHomePatterns() {
    const homeGrid = document.querySelector('.patterns-grid');
    if (homeGrid) {
        const allPatterns = getAllPatterns();
        homeGrid.innerHTML = allPatterns.slice(0, 6).map((pattern, index) => createPatternItem(pattern, index)).join('');
    }
}

// Refresh patterns when admin adds new products
